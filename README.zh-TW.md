...
|---|---|
| `0` | 成功——**但請見下方警告** |
| `1` | 所有失敗情形：未知旗標、未指定模式或指定兩個模式、找不到檔案、路徑不可讀寫、封存損壞或截斷、金鑰錯誤、密文遭竄改、RGB1 欄位驗證失敗、對壓縮封存執行 `-r` |
| `2` | 僅來自 `--zstd-level` 的驗證（超出範圍或非數字） |

`2` 並非通用的「參數值錯誤」類別——目前只有該旗標會回傳它。`-n abc` 與
`--strip-components abc` 屬同類錯誤，離開碼卻是 `1`。腳本請判斷 `!= 0`，不要針對
特定碼。

離開碼 `1` 也無法再細分：密語錯誤、封存遭竄改與檔案不存在，單看離開碼無從區別。
需讀取 stderr 才能分辨。

**關於 `0` 的警告：** 對任何小於 512 bytes 的輸入——包括被截斷的 `.tar.gz` 與隨機
位元組——`-t` 與 `-x` 會回傳 `0` 且無任何輸出，而 `bsdtar` 在同樣輸入下回傳 `1`
並印出 `Unrecognized archive format`。空檔案回傳 `0` 是正常慣例、與 `bsdtar` 一致；
但小於 512 bytes 的垃圾輸入並非如此，該問題記錄於
[`todo/todo.md`](todo/todo.md)。在其解決之前，請勿將 `-t` 回傳 `0` 視為封存完好的
證明。

`--zstd-level` 只有在同時指定 `--zstd` 時才生效。單獨給定時它會被接受、離開碼為 0，
並寫出一個未壓縮的純 tar——等級被丟棄且毫無警告。這與 `-r` / `-u` / `--delete`
不同，後三者會直接拒絕壓縮引擎旗標。可用 `--identify` 檢查：以
