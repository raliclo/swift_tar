...
`comparison.csv` was never regenerated while A was live, so no committed number
was ever affected; the next run uses the fixed binary.
`comparison.csv` 在 A 存在期間始終未重跑，故從未有任何入版數字受影響；下次執行
即採用已修正的 binary。
