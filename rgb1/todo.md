...
- **Never sample the opening 30 s, enforced hardest under 10 frames.** All six
  sampling scripts now take `max(duration/2, 30)` and reject a start inside the
  first 30 s when fewer than 10 frames are requested — including a hand-set
  `START`. A clip's first seconds hold fades, title cards and static holds: the
  8 frames at t=0 behind the 122 Mbps figure compressed to 7% of raw where real
  content gives 50%. A large sample dilutes that; a small one is dominated by it.

  The first version of this guard was broken in both directions: `FRAMES` was
  written without `$`, so the condition never saw the frame count and blocked
  48-frame runs too, and the error printed `[Error] 5=5s ... with only FRAMES
  frames`. Running only the case that should be blocked would have shown a
  correct-looking rejection. It took running the case that should *pass* to see
  it reject the wrong caller.
- **`--dry-run` on the four scripts that write committed records.** Testing the
  guards meant running for real, and one such test overwrote
  `test_interframe_output.txt` with data from t=5 s — the opening the guards
  exist to keep out. `--dry-run` validates tools, extraction, frame counts and
  the start rule, reports what it would write, and writes nothing. Verified both
  ways: a valid run reports and exits, an invalid one is still rejected, and the
  record is byte-identical afterwards in both cases.
- **`mixed_size_delta.zsh` runs on committed real frames.** Four fixtures at
  3-10 KB, generated by `fixtures/make_fixtures.zsh`, which records the source by
  **sha256 and size rather than by path** — an absolute path is right only on the
  machine that made it — and refuses to run against a different source unless
  forced. Provenance as something executable, not prose: a committed binary that
  cannot say where it came from is a fact nobody can check.

  Checking the generator reproduced the fixtures exposed that the pair meant to
  be *two different frames* was one frame twice: `f000` and `f001` of the corpus
  are byte-identical at full 1080p. The size discriminator still passed on that
  pair — it just stopped testing what it claims to. Fixed to frames 0 and 2, with
...
- **`test_encrypt.zsh` and `encrypt_windows_correctness.zsh` use a real frame when
  ...
  of that path must not depend on a 285 MB corpus that is not in the repository.
  Keys remain `/dev/urandom`; a key should be random.
- **`palette_vs_predictive.py` moved to the consecutive corpus and now writes its
  own record.** It printed to stdout and relied on the caller redirecting, so a
  re-run with output discarded left the committed file untouched and dated three
  days earlier — indistinguishable from a re-run that produced identical numbers.
  On the new corpus the verdict holds: palette +24.3% (was +23.5%), predictive
  −40.5% (was −40.8%), predictive beats palette by 52.2% (was 52.1%).

- **絕不取樣片頭 30 秒，格數少於 10 時檢查最嚴。** 六支取樣腳本現皆取
  `max(duration/2, 30)`，且在要求格數少於 10 時拒絕落在前 30 秒的起點——手動指定的
  `START` 也擋。影片開頭是淡入、標題卡與靜止畫面所在：產出 122 Mbps 那個數字的 t=0
  八格壓縮到原始的 7%，而真實內容為 50%。樣本大時該影響被稀釋，樣本小時由它主導。

  此守門的第一版兩個方向都是壞的：`FRAMES` 少寫 `$`，條件根本看不到格數，連 48 格也
  一併擋下，而錯誤訊息印成 `[Error] 5=5s ... with only FRAMES frames`。只跑「應該被擋」
  的案例會看到一個外觀正確的拒絕；是跑了「應該放行」的案例，才看見它擋錯了人。
- **四支會寫入版紀錄的腳本加上 `--dry-run`。** 先前要測守門就得真的跑一次，而其中一次
  把 `test_interframe_output.txt` 覆蓋成 t=5 秒的資料——正是那些守門要擋掉的片頭。
  `--dry-run` 會驗證工具、抽取、格數與起點規則，回報將寫入什麼，然後什麼都不寫。兩個
  方向皆驗證：有效參數回報後結束、無效參數仍被拒絕，且兩種情況紀錄檔皆位元組未變。
- **`mixed_size_delta.zsh` 改用入版的真實影格。** 四個 3-10 KB 的 fixture，由
  `fixtures/make_fixtures.zsh` 產生，該腳本以 **sha256 與檔案大小而非路徑**記錄來源
  ——絕對路徑只在產生它的那台機器上正確——且來源不符時除非強制否則拒絕執行。來源資訊
  應是可執行的而非文字描述：一個無從說明來源的入版二進位檔，是沒有人能查核的事實。

  為確認產生器忠實而比對重生成結果時，發現原本要當作「兩張不同影格」的那一對其實是
  同一張兩次：語料的 `f000` 與 `f001` 在完整 1080p 下位元組相同。體積判別式在該配對上
...
- **`palette_vs_predictive.py` 改用連續語料，並改為自行寫出紀錄。** 它原本只印到 stdout
  並依賴呼叫端重導，故一次把輸出丟棄的重跑會讓入版檔案原封不動、日期停在三天前——與
  「重跑後數字完全相同」無從區分。換語料後結論不變：調色盤 +24.3%（原 +23.5%）、
  預測式 −40.5%（原 −40.8%）、預測式勝過調色盤 52.2%（原 52.1%）。
