...
### 1. encrypt_windows_correctness.sh reports success but exits 1 / 報成功卻回傳 1
...
### 2. parallel_extract_correctness.zsh: mode checks lack a Windows guard / mode 檢查缺 Windows 守門
...
### 3. test_no_lzfse.sh fails silently on Windows / 在 Windows 上無聲失敗
...
### 4. parallel_extract_correctness.zsh takes 892 s on Windows / 在 Windows 上耗時 892 秒
